...
public class SearchByCondition extends HttpServlet{
  ...
  public void doPost(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException{
    ...
    if(radioMess.equals("ISBN")){
      ...
      condition= "SELECT * FROM bookForm where bookc_name LIKE '%"+searchMess+"%'";
    }
    else if(radioMess.equals("writer")){
      condition= "SELECT * FROM bookForm where book_writer LIKE '%"+searchMess+"%'";
    }
    else if(radioMess.equals("book_price")){
      double max=0, min=0;
      ...
      condition= "SELECT * FROM bookForm where "+"book_price <= "+max+" AND book_price >= "+min;
    }
  ...
  public void fail(HttpServletRequest request,HttpServletResponse response,String backNews){
    ...
    try{
      ...
      out.println("返回:");
      out.println("<a href=searchBook.jsp>查询书本</a>");
